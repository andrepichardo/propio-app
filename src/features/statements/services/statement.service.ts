...
async function nextStatementNumber(
  ...
  const count = await tx.statement.count({
    where: { ownerId, number: { startsWith: `STM-${year}-` } },
  });
  return `STM-${year}-${String(count + 1).padStart(4, '0')}`;
}
...
export const statementService = {
  ...
  async finalizePdf(params: {
    ...
    tenantName: string;
    propertyName: string;
    lines: { date: string; concept: string; amount: number }[];
  }): Promise<void> {
    try {
      ...
      const pdf = await renderStatementPdf({
        ...
        periodLabel: format(params.periodStart, 'MMMM yyyy'),
        ...
        nextDueDate: format(params.nextDueDate, 'PPP'),
